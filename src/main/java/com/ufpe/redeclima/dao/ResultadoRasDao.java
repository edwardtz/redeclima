...
public class ResultadoRasDao {
	...
	@Transactional
	public void save(ResultadosRAS resultadoRas) {
		entityManager.persist(resultadoRas);
	}
	
	@Transactional
	public void saveOrUpdate(ResultadosRAS resultadoRas) {
		entityManager.merge(resultadoRas);
	}
	
	public ResultadosRAS findById(long simulacaoId, long secaoId, Date data){
		
		ResultadosRAS resultadoRas;
		try{
			resultadoRas = (ResultadosRAS) entityManager.createQuery("from ResultadosRAS r where r.simulacao.id=:simulacaoId and r.secao.id=:secaoId and r.data=:data ")
	 ...
	 * */
	public List<ResultadosRAS> getResultados(Simulacao simulacao){
	...
	public List<ResultadosRAS> getResultados(Long simulacaoId, Date dataInicio, Date dataFim){
		
			return entityManager.createQuery("from ResultadosRAS r where r.simulacao.id=:simulacaoId and r.data>=:dataInicio and r.data<=:dataFim ")
					                    .setParameter("simulacaoId", simulacaoId)
	 ...
	 * */
	@SuppressWarnings("unchecked")
	public List<ResultadosRAS> getResultados(Long simulacaoId, Long secaoId){
		
			return entityManager.createQuery("from ResultadosRAS r where r.simulacao.id=:simulacaoId and r.secao.id=:secaoId ")
					                    .setParameter("simulacaoId", simulacaoId)
					                    .setParameter("secaoId", secaoId)
					                    .getResultList();
	}
	 ...
	 * */
	@Transactional
	public void salvarArquivosTemporais(Simulacao simulacao){
	 ...
	 * */
	@Transactional
	public void salvarResultados(Simulacao simulacao){
		
		String pathArquivoCSV = Paths.get(pathWorkspaces, simulacao.getUsuario().getLogin().toUpperCase(), simulacao.getBacia().getNome().toUpperCase(), "HEC", "tmpRAS", simulacao.getBacia().getNome() + "BulkFileCSV.csv").toString();
		
		entityManager.createNativeQuery("copy t_resultados_ras (simulacao_id, secao_id, data, elevacao, fluxo) from '" + pathArquivoCSV + "' WITH DELIMITER ','").executeUpdate();
	}
	
	@Transactional
	public void removerResultados(Simulacao simulacao){
		entityManager.createQuery("delete from ResultadosRAS r where r.simulacao.id=:simulacaoId").setParameter("simulacaoId", simulacao.getId()).executeUpdate();
	}
