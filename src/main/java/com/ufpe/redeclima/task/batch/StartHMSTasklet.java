...
@Component
@StepScope
public class StartHMSTasklet extends StepExecutionListenerSupport implements Tasklet, InitializingBean {

	private static final Logger logger = LoggerFactory.getLogger(StartHMSTasklet.class);
	...
	@Value("${parameter.hms_executable_path}")
	private String hmsExecutablePath;
	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
		...
		workingDirectory = new File(Paths.get(hmsExecutablePath).toString());
		
		command = "cmd /c start /wait "+ "/D\"" + Paths.get(hmsExecutablePath).toString() + "\" HEC-HMS.cmd -s" + " \""+ Paths.get(pathWorkspaces, simDto.getUsuario().getLogin().toUpperCase(), simDto.getBacia().getNome().toUpperCase(), "HEC", "HMS", simDto.getBacia().getNome() + "SimulaEventosHMS.script").toString() +"\" ^&^& exit";
		...
		while (true) {
			...
			if (systemCommandTask.isDone()) {
				...
				logger.error("Execução do modelo HMS não concluiu no tempo contemplado no timeout");
				throw new SimulacaoException("Execução do modelo HMS não concluiu no tempo contemplado no timeout");
			}
