...
@Component
@StepScope
public class StartRASTasklet extends StepExecutionListenerSupport implements Tasklet, InitializingBean{

	private static final Logger logger = LoggerFactory.getLogger(StartRASTasklet.class);
	
	/* Diretorio dos workspaces dos usuarios */
	@Value("${parameter.path_workspaces}")
	private String pathWorkspaces;
	
	/* Diretorio do script de execução de RAS */
	@Value("${parameter.ras_script_path}")
	private String pathScriptRas;
	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
		...
		String nomeProjetoRas = workspace.identificarNomeProjetoRAS(simDto.getBacia());
		
		workingDirectory = new File(Paths.get(pathScriptRas).toString());
		
		command = "ExecuteRAS.exe " + Paths.get(pathWorkspaces, simDto.getUsuario().getLogin().toUpperCase(), simDto.getBacia().getNome().toUpperCase(), "HEC", "RAS", nomeProjetoRas).toString();
		...
		while (true) {
			...
			if (systemCommandTask.isDone()) {
				...
				logger.error("Execução do modelo RAS não concluiu no tempo contemplado no timeout");
				// Saindo da zona critica
				System.out.println("Saindo da zona critica " + command);
				semaphoreRAS.getAvailable().release();
				
				throw new SimulacaoException("Execução do modelo RAS não concluiu no tempo contemplado no timeout");
			}
